"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
/// <reference path="../../../../vendor.d.ts"/>
var core_1 = require('@angular/core');
var common_1 = require('@angular/common');
var MessageService_1 = require('./../../../services/MessageService');
var AccountService_1 = require('./../../../services/AccountService');
var DashboardComponent = (function () {
    // Constructor
    function DashboardComponent(messageService, accountService) {
        this.messageService = messageService;
        this.accountService = accountService;
        this.activeCtrlCenter;
        this.currentCtrlCenter;
    }
    // Functions
    DashboardComponent.prototype.ngOnInit = function () {
        this.setActiveCtrlCenter("profile");
    };
    DashboardComponent.prototype.ngAfterViewInit = function () {
    };
    DashboardComponent.prototype.setActiveCtrlCenter = function (ctrlCenter) {
        this.activeCtrlCenter = {
            profile: false,
            versions: false,
            blog: false,
            projects: false,
            account: false
        };
        this.activeCtrlCenter[ctrlCenter] = true;
        this.currentCtrlCenter = ctrlCenter;
    };
    DashboardComponent.prototype.openCreateForm = function () {
        this.messageService.broadcast('create', [{ ctrlCenter: this.currentCtrlCenter }]);
    };
    DashboardComponent.prototype.mockToggleLogin = function () {
        this.accountService.loggedIn ? this.accountService.logout() : this.accountService.mockLogin();
    };
    DashboardComponent = __decorate([
        core_1.Component({
            selector: 'dashboard',
            styles: [],
            directives: [
                common_1.CORE_DIRECTIVES,
            ],
            templateUrl: '/client/app/admin/components/baseComponent/baseComponent.html',
        }), 
        __metadata('design:paramtypes', [MessageService_1.MessageService, AccountService_1.AccountService])
    ], DashboardComponent);
    return DashboardComponent;
}());
exports.DashboardComponent = DashboardComponent;
//# sourceMappingURL=baseComponent.js.map